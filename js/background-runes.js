...
@keyframes backgroundRuneFade {
    ...
    const createRune = (layer, options = {}) => {
        ...
        const runeRotation = Math.random() > 0.5 ? "180deg" : "0deg";
        ...
        rune.style.setProperty("--rune-rotate", runeRotation);
        rune.style.setProperty("--rune-scale", randomBetween(0.82, 1.16));
        rune.style.setProperty("--rune-drift-x", `${randomBetween(-55, 55)}px`);
        rune.style.setProperty("--rune-drift-y", `${randomBetween(-80, 80)}px`);
        rune.style.setProperty("--rune-fade-duration", `${randomBetween(8, 12)}s`);
        rune.style.setProperty("--rune-drift-duration", `${randomBetween(10, 22)}s`);
        rune.style.setProperty("--rune-delay", `${randomBetween(-8, 0)}s`);

        layer.appendChild(rune);
        layer.appendChild(rune);

        rune.addEventListener("animationiteration", (event) => {
            if (event.animationName !== "backgroundRuneFade") return;

            randomiseRunePosition(rune);
        });
    };
    ...
    const randomiseRunePosition = (rune) => {
        const runeRotation = Math.random() > 0.5 ? "180deg" : "0deg";
    ...
    const initialiseBackgroundRunes = () => {
        ...
        createRuneLayer({
            ...
            count: prefersReducedMotion ? 18 : 48,
            colours: bodyColours,
            baseOpacity: 0.84,
            sizeMinimum: 0.85,
            sizeMaximum: 2.3
